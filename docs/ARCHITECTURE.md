...
- **Feature Flag Gating (`show_risk_tier`):** Public display of listing `risk_tier` badges and search filters across marketplace cards, listing detail pages, and widgets is conditionally gated behind the `show_risk_tier` feature flag via `useFeatureFlags` composable. Moderators always have access to `risk_tier` controls regardless of the flag state.
