...
- **Web Server & PaaS Configuration:** The Chabokan PaaS Vue host ignores this repository's `nginx.conf` via `.chabokignore` to preserve Chabokan's default Vue web root (`root /app/dist; try_files $uri $uri/ /index.html =404;`) since the Docker container root path (`/usr/share/nginx/html/dist`) differs from `/app/dist`. Panel environment variables `VITE_USE_MOCK=false` and `VITE_API_BASE_URL=https://chequeyar-back.chbkn.dev/api/v1` are configured in the Chabokan service panel because Chabokan runs `npm run build` on the server during deployment.
