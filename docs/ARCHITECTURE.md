...
- **Public Chrome & Landing Module (`src/features/landing/`):**
  ...
  - Reusable layout and presentation primitives under `src/features/landing/components/`:
    - `LandingSectionShell.vue`: Provides unified vertical rhythm, container constraints, eyebrow pills, section titles/subtitles, and background variants (`default`, `muted`, `elevated` with subtle gradient separators).
    - `LandingSurfaceCard.vue`: Standardized card surface token with optional highlight borders, top accent bars, and elevation hover states.
