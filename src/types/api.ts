...
export interface ChequeListing { id: number; owner_id: number; issuer_profile: IssuerProfile; bank: BankSummary | null; bank_name: string; cheque_serial_number: string; face_amount: string; due_date: string; issuer_type: IssuerType; issuer_name: string; issuer_national_id: string; description: string; suggested_discount_rate: string | null; risk_tier: 'low' | 'medium' | 'high' | null; status: ListingStatus; rejection_reason: string; rejection_code: string | null; resubmit_count: number; documents?: ListingDocumentItem[]; created_at: string; updated_at: string; }
export interface CreateListingRequest { issuer?: number; bank: string; cheque_serial_number: string; face_amount: number | string; due_date: string; issuer_type: IssuerType; issuer_name: string; issuer_national_id: string; description?: string; suggested_discount_rate?: string | null; }
export type UpdateListingRequest = Partial<CreateListingRequest>;
export interface ListingFilters { risk_tier?: 'low' | 'medium' | 'high'; min_amount?: number; max_amount?: number; max_days_to_due?: number; issuer_type?: 'legal' | 'natural'; bank?: string; bank_name?: string; ordering?: string; page?: number; page_size?: number; }

export interface MarketplaceListing { id: number; owner_id: number; issuer_profile: IssuerProfile; bank: BankSummary | null; bank_name: string; cheque_serial_number: string; face_amount: string; due_date: string; issuer_type: IssuerType; issuer_name: string; issuer_national_id: string; description: string; suggested_discount_rate: string | null; risk_tier: 'low' | 'medium' | 'high' | null; status: ListingStatus; days_to_due: number; interest_count: number; published_at: string | null; created_at: string; updated_at: string; }
export interface MarketplaceLatestListing { id: number; issuer_profile: IssuerProfile; bank: BankSummary | null; bank_name: string; face_amount: string; due_date: string; issuer_type: IssuerType; suggested_discount_rate: string | null; risk_tier: 'low' | 'medium' | 'high' | null; status: ListingStatus; days_to_due: number; created_at: string; }

export interface UserSummary { id: number; username: string; name: string; phone?: string; email?: string; }
export interface ListingSummary { id: number; bank?: BankSummary | null; bank_name: string; face_amount: string; due_date: string; status: ListingStatus; created_at: string; updated_at: string; }
